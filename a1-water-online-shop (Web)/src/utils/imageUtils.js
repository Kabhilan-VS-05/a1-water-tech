...
const CATEGORY_FALLBACKS = {
  purifiers:    encodeURI('/Purifiers Explore.png'),
  commercial:   encodeURI('/Commercial.png'),
  filters:      encodeURI('/Filters.png'),
  accessories:  encodeURI('/Accessories.png'),
  services:     encodeURI('/Services.png'),
}
...
export function getProductImage(product) {
  // 1. Always prioritize the actual image URL from the AWS database if available
...
export function handleImageError(event, type = 'product') {
  ...
  event.currentTarget.src = type === 'service' ? FALLBACK_SERVICE : FALLBACK_PRODUCT
}
